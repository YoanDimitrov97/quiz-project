            ...
            <div v-if="userId" class="create_quiz" v-bind:test="quizId">
                ...
                <div class="questions" ref="container">
                    <NewQuizQuestion
                        :id="questionNum"
                        :quizId="quizId"
                        :questionData="questionData[questionNum]"
                        :key="questionNum"
                        v-for="questionNum in questionNum"
                        @deleteQuestion="deleteQuestion"
                    />
                </div>
                <div class="add-save-container">
                    <div class="add_question" v-on:click="addQuestion">
                        <p>Add New Question</p>
                        <img src="~@/assets/images/add.svg" />
                    </div>
                    <div class="save_quiz" v-on:click="saveQuiz">
                        <p>{{ editButton.name }} Quiz</p>
...
.special-background {
    height: 100vh;
    width: 100vw;
    background: transparent linear-gradient(1deg, #3c4568 0%, #3c4568 100%) 0%
...
.wrapper {
    ...
    .create_quiz {
        ...
        margin: 10px auto;
        display: grid;
        grid-template-rows: $categ_h $categ_h calc(
                100% - #{$categ_h * 2} $categ_h $categ_h
            ); // This must be fixed .... !
        gap: 10px;
        #quiz_pic {
            width: 0px;
